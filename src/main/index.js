...
ipcMain.on('login', (event, data) => {
    focusWindowClose()
    mainWin = NewWindow({ width: 1000, height: 800, minHeight: 650, minWidth: 800, backgroundColor: '#F6F6F6' }, 'main')
})

// 登出
ipcMain.on('logout', (event, data) => {
    focusWindowClose()
    loginWin = NewWindow({ height: 560, width: 380, backgroundColor: '#F6F6F6' }, 'login')
})
...
ipcMain.on('updateStatus', (event, data) => {
    updateWin = NewWindow({ width: 600, height: 420, backgroundColor: '#F6F6F6' }, 'update')
})

// 打开历史记录
ipcMain.on('openHistory', (event, data) => {
    historyWin = NewWindow({ width: 1000, height: 650, backgroundColor: '#F6F6F6' }, 'history')
})

// 打开用户设置
ipcMain.on('openSetting', (event, data) => {
    settingWin = NewWindow({ width: 600, height: 450, backgroundColor: '#F6F6F6' }, 'setting')
})
...
// 屏幕截图操作 
function captureOperate(e, { type = 'start', screenId, url, path, scale } = {}) {
    if (type === 'start') {
        captureScreen()
