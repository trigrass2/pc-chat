...
export class CaptureEditor extends Event {
    ...
    async init() {
        ...
        // this.$bg.style.backgroundSize = `${this.screenWidth}px ${
        // this.screenHeight}`
        this.$bg.style.backgroundSize = 'contain'
        let canvas = document.createElement('canvas')
        let ctx = canvas.getContext('2d')
        let img = await new Promise(resolve => {
            let img = new Image()
            img.src = this.imageSrc
            // complete只是HTMLImageElement对象的一个属性，可以判断图片加载完成，不管图片是不是有缓存；
            // 而onload则是这个Image对象的load事件回调，当图片加载完成后执行onload绑定的函数。
            if (img.complete) {
                resolve(img)
            } else {
    ...
    getImageScale() {
      if (this.selectRect) {
        return this.selectRect.w / this.selectRect.h
      } else {
        return 1
      }
    }
